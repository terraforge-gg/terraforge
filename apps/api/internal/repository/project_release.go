...
func (r *projectReleaseRepository) FindReleaseByIdWithDependencies(ctx context.Context, q database.Querier, id string) (*models.ProjectRelease, error) {
	query := `
		...
		LEFT JOIN "project_release_dependency" d ON v."id" = d."releaseId"
	...
	for rows.Next() {
		...
		if depId.Valid {
			...
			dep.DependencyProjectId = depProjectId.String
...
func (r *projectReleaseRepository) FindReleasesByProjectIdWithLoaderVersion(ctx context.Context, q database.Querier, projectId string) ([]models.ProjectRelease, error) {
	...
	for rows.Next() {
		...
		var loaderVersionLabelStr sql.NullString
		var loaderVersionBuildType sql.NullString
		...
		if err := rows.Scan(
			...
			&loaderVersionLabelStr,
			&loaderVersionBuildType,
			&loaderReleasedAt,
			&loaderUpdatedAt,
		); err != nil {
			return nil, err
		...
		if loaderId.Valid {
			...
			lv.VersionLabel = loaderVersionLabelStr.String
			lv.BuildType = models.LoaderVersionBuildType(loaderVersionBuildType.String)
